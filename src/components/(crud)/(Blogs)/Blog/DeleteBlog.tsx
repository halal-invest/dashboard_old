...
interface IProps {
    rowSelected: IBlogsInclude[],
    setRowSelected: any,
    refetch: () => void
}

const DeleteBlog = ({ rowSelected, refetch, setRowSelected }: IProps) => {

    const [dialog, setDialog] = useState<boolean>(false);
    const [select, setSelect] = useState<IBlogsInclude[]>([]);
    ...
    const deleteHandler = async () => {
        ...
        const id = select && select.map((sl: IBlogsInclude) => sl.id);


        try {
            const { data } = await axios.delete("/api/admin/blogs", {
                data: { id }
            });
