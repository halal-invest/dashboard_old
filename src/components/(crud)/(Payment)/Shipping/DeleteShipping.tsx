...
interface IProps {
    rowSelected: IShippingType[],
    setRowSelected: any,
    refetch: () => void
}

const DeleteShipping = ({ rowSelected, refetch, setRowSelected }: IProps) => {

    const [dialog, setDialog] = useState<boolean>(false);
    const [select, setSelect] = useState<IShippingType[]>([]);
    ...
    const deleteHandler = async () => {
        ...
        try {
            const { data } = await axios.delete("/api/admin/shipping", {
                data: { id }
            });
