...
import { ICategory, IPaymentMethodType, ISliderType } from '@/types/common';
import DeleteModalButton from '@/components/Common/(Button)/DeleteModalButton';


interface IProps {
    rowSelected: ISliderType[],
    setRowSelected: any,
    refetch: () => void
}

const DeleteSlider = ({ rowSelected, refetch, setRowSelected }: IProps) => {

    const [dialog, setDialog] = useState<boolean>(false);
    const [select, setSelect] = useState<ISliderType[]>([]);
    ...
    const deleteHandler = async () => {
        ...
        try {
            const { data } = await axios.delete("/api/admin/slider", {
                data: { id }
            });
