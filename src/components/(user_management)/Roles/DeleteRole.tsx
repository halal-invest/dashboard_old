...
const DeleteRole = ({ rowSelected, refetch, setRowSelected }: { rowSelected: IProps[] | null; setRowSelected: any; refetch: () => void }) => {
    const [dialog, setDialog] = useState<boolean>(false);
    const [select, setSelect] = useState<IProps[]>([]);
    ...
    const deleteHandler = async () => {
        ...
        try {
            const { data } = await axios.delete('/api/user_management/roles', {
                data: { id }
            });
